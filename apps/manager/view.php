...
function AddUserView() {
    ...
    if (CheckAccess($session, 'AddUserView', 'read')) {
        ...
        FROM user';
        $table_content = QueryEngine($sql, '', '', 0);
        /* Create table */
...
function AssignUserView() {
    ...
    if (CheckAccess($session, 'AssignUserView', 'read')) {
        ...
        FROM user';
        $dropdown_element = QueryEngine($sql, '', '', 0);
        ...
        $table_content = QueryEngine($sql, array(&$user, &$user), 'ss', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function DeassignUserView() {
    ...
    if (CheckAccess($session, 'DeassignUserView', 'read')) {
        ...
        INNER JOIN user USING (user_id)
        WHERE username = ?';
        $table_content = QueryEngine($sql, array(&$user), 's', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function GrantPermissionView() {
    ...
    if (CheckAccess($session, 'GrantPermissionView', 'read')) {
        ...
        $table_content = QueryEngine($sql, array(&$role, &$role), 'ss', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function RevokePermissionView() {
    ...
    if (CheckAccess($session, 'RevokePermissionView', 'read')) {
        ...
        INNER JOIN role USING (role_id)
        WHERE role_id = (SELECT role_id FROM role WHERE name = ?)';
        $table_content = QueryEngine($sql, array(&$role), 's', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function CreateSessionView() {
    ...
    if (CheckAccess($session, 'CreateSessionView', 'read')) {
        $sql = 'SELECT DISTINCT user_session.session_id AS ' . localize('Session') . ', 
        CONCAT(user.family_name, ", ", user.first_name) AS ' . localize('User') . '
        FROM user
        INNER JOIN user_session USING (user_id)';
        $table_content = QueryEngine($sql, '', '', 0);
        /* Create table */
        $create_page_array = array(
...
function DeleteSessionView() {
    ...
    if (CheckAccess($session, 'DeleteSessionView', 'read')) {
        ...
        user_session.session_id AS ' . localize('Session') . ', 
        CONCAT(user.family_name, ", ", user.first_name) AS ' . localize('User') . '
        FROM user
        INNER JOIN user_session USING (user_id)
...
function AddActiveRoleView() {
    ...
    if (CheckAccess($session, 'AddActiveRoleView', 'read')) {
        ...
        if (empty($user_session)) {
            $sql = 'SELECT MIN(CONCAT(user.username, "+", session.name)) AS user_session FROM user 
            INNER JOIN user_session USING (user_id)
            INNER JOIN session USING (session_id)';
            $results = QueryEngine($sql, '', '', 0);
            if (!empty($results)) {
        ...
        INNER JOIN user_session USING (session_id)
        WHERE user_session.user_id = (SELECT user_id FROM user WHERE username = ?)
        UNION
        ...
        $table_content = QueryEngine($sql, array(&$username, &$username), 'ss', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function DropActiveRoleView() {
    ...
    if (CheckAccess($session, 'DropActiveRoleView', 'read')) {
        ...
        INNER JOIN user_session USING (session_id)
        WHERE user_session.user_id = (SELECT user_id FROM user WHERE username = ?)';
        $table_content = QueryEngine($sql, array(&$username), 's', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function SessionRolesView() {
    ...
    if (CheckAccess($session, 'SessionRolesView', 'read') && CheckAccess($session, 'SessionRoles', 'read')) {
        ...
        /* If no user has been set select the lowest one */
        if (empty($user_session)) {
            $sql = 'SELECT MIN(name) AS user_session FROM session';
            $results = QueryEngine($sql, '', '', 0);
            if (!empty($results)) {
                $user_session = $results[0]['user_session'];
            }
...
function SessionPermissionsView() {
    ...
    if (CheckAccess($session, 'SessionPermissionsView', 'read') && CheckAccess($session, 'SessionPermissions', 'read')) {
        /* Retrieve list of all active session */
        $sql = 'SELECT DISTINCT session.name AS user_session, CONCAT(user.family_name, ", ", user.first_name) AS ' . localize('User') . '
        FROM user
        INNER JOIN user_session USING (user_id)
        INNER JOIN session USING (session_id)';
        $dropdown_element = QueryEngine($sql, '', '', 0);
...
function AddPermissionView() {
    ...
    if ($submitted && empty($ajax)) {
        ...
        /* Run through the error array and display any error messages */
    	  if (is_array($error_array)){
            foreach ($error_array as $error_value) {
                $status_message .= '<p>' . $error_value . '<\/p>';
            }
    ...
    if (CheckAccess($session, 'AddPermissionView', 'read')) {
        ...
        FROM object';
        $dropdown_object = QueryEngine($sql, '', '', 0);
        /* Query for filling the dropdown list */
        $sql = 'SELECT name AS operation_name, name 
        FROM operation';
        $dropdown_operation = QueryEngine($sql, '', '', 0);
        /* Create table */
        $create_page_array = array(
            'status_message' => $status_message,
...
function Menu() {
    if (!empty($_SERVER['PHP_AUTH_USER'])) {
        ...
        if (logIn("Menu")) {
            ...
            $role = getCleanedRoles(AssignedRoles($user));
            ...
            $permissions_array = personalisedMenuItems(UserPermissions($user));
