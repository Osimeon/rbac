    ...
    if (!empty($user) && !empty($roles)) {
        ...
        while (list($key, $val) = each($roles)) {
            ...
            if (empty($results)) {
                ...
                VALUES ((SELECT session_id FROM user_session WHERE user_id = (SELECT user_id FROM user WHERE username = ?)), 
...
function CreateSession($user='', $session='') {
    ...
    $sql = 'SELECT session.session_id, user_session.user_id, session_role.role_id, UNIX_TIMESTAMP(session.created) AS unix_timestamp 
    FROM session 
    INNER JOIN user_session USING (session_id)
    INNER JOIN session_role USING (session_id)
    WHERE session.name = ?';
    ...
    if (empty($session_id) && empty($user_id) && empty($role_id)) {
        ...
        if ($query_success) {
            ...
            $sql = 'INSERT INTO session (name, created) VALUES (?, NOW())';
            $results = QueryEngine($sql, array(&$session), 's', 0);
            if (!empty($results)) {
                $query_success = FALSE;
            }
...
function AddActiveRole($user='', $session='', $roles=array()) {
    ...
    if (!empty($user) && !empty($session) && !empty($roles)) {
        ...
        while (list($key, $val) = each($roles)) {
            ...
            INNER JOIN user_session USING (session_id)
            WHERE user_id = (SELECT user_id FROM user WHERE username = ?)
            AND session_id = (SELECT session_id FROM session WHERE name = ?)
            AND role_id = (SELECT role_id FROM role WHERE name = ?)';
            ...
            $sql = 'SELECT session_id FROM user_session WHERE user_id = (SELECT user_id FROM user WHERE username = ?)';
            $results = QueryEngine($sql, array(&$user), 's', 0);
            if (!empty($results)) {
                $session_id = (int) $results[0]['session_id'];
            }
...
function DropActiveRole($user='', $session='', $roles=array()) {
    ...
    if (!empty($user) && !empty($session) && !empty($roles)) {
        ...
        while (list ($key, $val) = each($roles)) {
            ...
            AND user_session.session_id = (SELECT session_id FROM session WHERE name = ?)
